...
function AdminStudents() {
  const API_BASE = "https://leaplearning.onrender.com";
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/students/admin/${id}`, {
        method: "DELETE",
      });
  ...
  const handleUploadInvoice = async (student, file) => {
    ...
    try {
      ...
      form.append("payment_method", "Admin Upload");
      ...
      if (res.ok) {
        ...
        if (fileRefs.current[student.id]) {
          fileRefs.current[student.id].value = "";
        }
      } else {
        await Swal.fire({
          icon: "error",
  ...
  const renderDocLink = (url) => {
    return url ? (
      <a href={url} target="_blank" rel="noreferrer" style={docBtn}>
        <FaEye size={12} />
      </a>
    ) : (
      <span style={naText}>N/A</span>
    );
  };
  ...
  const filteredStudents = students.filter((student) => {
    ...
    if (docFilter === "uploaded") {
      matchDoc =
        student.academic || student.idproof || student.resume || student.other;
    }
    ...
    return matchSearch && matchProgram && matchDoc;
  });

  const uniquePrograms = [
    ...new Set(students.map((x) => x.program).filter(Boolean)),
  ];
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, programFilter, docFilter, rowsPerPage]);
          ...
          <tbody>
            {paginatedStudents.map((student) => (
              <tr key={student.id}>
                ...
                <td style={tdWide}>{student.email}</td>

                <td style={td}>{student.phone}</td>

                <td style={td}>{student.program}</td>

                <td style={td}>₹{student.total_fee}</td>

                <td style={td}>{student.invoice_number}</td>

                <td style={tdCenter}>{renderDocLink(student.academic)}</td>

                <td style={tdCenter}>{renderDocLink(student.idproof)}</td>

                <td style={tdCenter}>{renderDocLink(student.resume)}</td>

                <td style={tdCenter}>{renderDocLink(student.other)}</td>

                <td style={td}>
                  <div style={actionWrap}>
                    <button onClick={() => handleEdit(student)} style={editBtn}>
                      <FaEdit size={11} />
                    </button>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleUploadInvoice(student, e.target.files[0])
                      }
                    />
